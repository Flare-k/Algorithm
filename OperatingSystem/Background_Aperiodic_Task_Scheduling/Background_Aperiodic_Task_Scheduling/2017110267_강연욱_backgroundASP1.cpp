...
float average_waiting_time(int n, vector<float> aperiodIdx, vector<int> arrivalTime);  //Average Waiting Time을 계산해주는 함수
    ...
    for(int index=0; index < aperiod.size(); index++){
        ...
        for(int i=aT-1; i<=hyperPeriod; i++){
            if(check[i] == false){
                check[i] = idx;
                aC--;
                aperiodIdx.push_back(i);    //ap computation index를 저장한다.
                if(aC == 0){
                    break;
                }
            }
...
float average_waiting_time(int n, vector<float> aperiodIdx, vector<int> arrivalTime) {
    float sum = 0;
    for (int i = 0; i < aperiodIdx.size(); i++){
