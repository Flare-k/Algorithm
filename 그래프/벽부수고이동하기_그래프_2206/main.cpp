...
int BFS() {
   ...
   q.push(make_pair(make_pair(0, 0), make_pair(0, 1)));  // x,y좌표 & wall 부순횟수, 현재 칸수
   visit[0][0][0] = true;
   while (!q.empty()) {
      ...
      for (int i = 0; i < 4; i++) {
         ...
         if (0 <= nx && nx < row && 0 <= ny && ny < col) {
            if (arr[nx][ny] && !crash) {  // 벽이면서 아직 부순적이 없다면
               ...
               q.push(
                   make_pair(make_pair(nx, ny), make_pair(crash + 1, cnt + 1)));
            } else if (!arr[nx][ny] &&
                       !visit[nx][ny]
                             [crash]) {  //벽이 아니지만 아직 방문한 적이 없다면
               visit[nx][ny][crash] = true;
               q.push(make_pair(make_pair(nx, ny), make_pair(crash, cnt + 1)));
            }
         }
      }
   }
