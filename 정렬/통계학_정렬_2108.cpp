...
bool comp(pair<int, int> a, pair<int, int> b) {
   if (a.first > b.first)  // 만약 a의 cnt가 b의 cnt보다 크다면
      return true;
...
int main() {
   ...
   while (idx != (arr.size() + 1)) {
      if (arr[idx] == temp) {  // temp의 값과 arr[idx]의 값이 같으면 중복이므로
         ...
         res.push_back(
             make_pair(cnt, temp));  // 만약 다음 arr[idx]값과 다르다면 현재의
   ...
   sort(res.begin(), res.end(),
        comp);  // 중복횟수가 높은 순으로 정렬하고, 중복횟수가 같다면
                // 오름차순으로 정렬한다.
   int ans = 0;
   if (res.size() ==
       1)  //만약 하나의 수만 입력했다면 res에도 하나만 저장되어있을 것이다.
      ans = res[0].second;
   else {
